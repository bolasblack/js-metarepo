import { getPreEmitDiagnostics, flattenDiagnosticMessageText, CompilerOptions } from 'typescript'
...
export default function dtsGenerate(
  ...
  return plugin('dtsGenerate', ({ logPath }) => async ({ files }: StartFilesProps) => {
    ...
    allDiagnostics.forEach(diagnostic => {
      if (diagnostic.file) {
        ...
        const pos = `${diagnostic.file.fileName} (${line + 1},${character + 1})`
